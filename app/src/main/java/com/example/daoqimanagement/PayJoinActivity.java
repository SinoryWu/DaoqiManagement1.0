...
import com.example.daoqimanagement.bean.PayInfoResponse;
import com.example.daoqimanagement.bean.PayJoinResponse;
...
public class PayJoinActivity extends AppCompatActivity implements View.OnClickListener {
    // 申请相机权限的requestCode
    ...
    //用于保存拍照图片的uri
    private Uri mCameraUri,mAlbumUri;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getPaymentInfoRes(Api.URL+"/v1/payment/info?type=1");
        Log.d("TAG", String.valueOf(mIvPhotoTrue.getVisibility()));
        mLlArrowLeft.setOnClickListener(new OnMultiClickListener() {
            @Override
            public void onMultiClick(View view) {
                hideSoftKeyboard(view);
                LoginOutDialog loginOutDialog = new LoginOutDialog(PayJoinActivity.this,R.style.CustomDialog);
                loginOutDialog.setConfirm("是的", new LoginOutDialog.IOnConfirmListener() {
                    @Override
                    public void onConfirm(LoginOutDialog dialog) {
                        Intent intent = new Intent(PayJoinActivity.this,LoginActivity.class);
                        startActivity(intent);
                        ActivityCollector.removeActivity(PayJoinActivity.this);
                        finish();
                    }
                }).show();
            }
    ...
    public void initView(){
        ...
        mLlCamera = findViewById(R.id.pay_join_ll_camera);
        mLlArrowLeft = findViewById(R.id.pay_join_ll_arrowleft);
        mIvPhotoTrue = findViewById(R.id.pay_join_iv_photo_true);
        mIvPhotoFalse = findViewById(R.id.pay_join_iv_photo_false);
        mTvPhotoToast = findViewById(R.id.pay_join_tv_photo);
        mTvPhotoAgain = findViewById(R.id.pay_join_tv_photo_again);
        mBtnSubmit = findViewById(R.id.pay_join_btn_submit);
        mTvBusinessName = findViewById(R.id.pay_join_tv_enterprise);
        mTvBankNo = findViewById(R.id.join_in_tv_bank_number);
        mTvBank = findViewById(R.id.pay_join_tv_bank);
        mTvAmount = findViewById(R.id.pay_join_tv_money_small);
        mTvBigVoucher = findViewById(R.id.pay_join_tv_money_capital);
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.pay_join_ll_camera:
                showPuPopWindow(view);
                break;
            case R.id.pay_join_tv_photo_again:
                showPuPopWindow(view);
                break;

            case R.id.pay_join_btn_submit:
                if (pictureId.equals("00000")){
                    ToastUtils.showTextToast2(PayJoinActivity.this,"请上传转账凭证");
                }else {
                    payJoinPost(pictureId);
                }
                break;
        }
    ...
    public void showPuPopWindow(View popupView){
        if (popupWindow == null) {
            ...
            popupWindow.showAtLocation(PayJoinActivity.this.findViewById(R.id.pay_join_bottom_view), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
...
//                        bitmap = BitmapFactory.decodeFile(getPath(this, mAlbumUri));
                        bitmap = compressImage1(getPath(this, mAlbumUri));
                        File file = createFilePic(this, bitmap);
                        postUpLoadPhoto(Api.URL+"/upload", file);
                    } else {
...
//        Response response = call.execute();
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.showTextToast2(PayJoinActivity.this, "网络请求失败");
                    }
                });
            }
                    ...
                    public void run() {
                        ...
                        if (paymentInfoResponse.getCode() == 0) {
                            ...
                            if (dialogTokenIntent == null) {
                                dialogTokenIntent = new DialogTokenIntent(PayJoinActivity.this, R.style.CustomDialog);
                            ...
                            ToastUtils.showTextToast2(PayJoinActivity.this, msg);
                        }
                    }
                });
    ...
    public void payJoinPost(String image){
        HashMap<String, String> map = new HashMap<>();
        map.put("image", image);//18158188052

        String url = Api.URL+"/v1/payment/addFranchiseFee";
        postPayJoin(url,map);

    }

    public void postPayJoin(String url, HashMap<String, String> map) {
        //创建OK
        ...
        call.enqueue(new Callback() {   //异步
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        Gson gson = new Gson();
                        PayJoinResponse payJoinResponse = gson.fromJson(res,PayJoinResponse.class);
                        if (payJoinResponse.getCode() == 0){
                            Intent intent = new Intent(PayJoinActivity.this,UserInterFaceActivity.class);
                            startActivity(intent);
                           ToastUtils.showTextToast2(PayJoinActivity.this,"提交成功");
                            ActivityCollector.removeActivity(PayJoinActivity.this);
                            finish();
                        }else if (payJoinResponse.getCode() == 10009) {
                            if (dialogTokenIntent == null) {
                                dialogTokenIntent = new DialogTokenIntent(PayJoinActivity.this, R.style.CustomDialog);
                                dialogTokenIntent.setTitle("提示").setMessage("您好，您的登陆信息已过期，请重新登陆!").setConfirm("确认", new DialogTokenIntent.IOnConfirmListener() {
                                    @Override
                            ...
                            ToastUtils.showTextToast2(PayJoinActivity.this,payJoinResponse.getMsg());
                        }
...
//                .url("http://yun918.cn/study/public/index.php/file_upload.php")
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (json.equals("") || json == null){
                            ToastUtils.showTextToast2(PayJoinActivity.this,"上传失败，请重新拍摄");
                            ...
                            if (uploadPhotoResponse.getCode() == 0) {
                                ...
                                pictureId = String.valueOf(uploadPhotoDateResponse.getPictureId());
                                ...
                                mIvPhotoTrue.setOnClickListener(new OnMultiClickListener() {
                                    @Override
                                    public void onMultiClick(View view) {
                                        CheckPhotoBitmapDialog checkPhotoBitmapDialog = new CheckPhotoBitmapDialog(PayJoinActivity.this,R.style.CustomDialogPhoto, bitmap);
    ...
    private void checkPermissionAndAlbum() {
        if (ContextCompat.checkSelfPermission(getApplication(), Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            ...
            //没有权限，申请权限。
            ActivityCompat.requestPermissions(PayJoinActivity.this,new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_ALBUM_REQUEST_CODE);
        }
    }
...
//                Uri contentUri = ContentUris.withAppendedId(Uri.parse("content://media/external/images/media"),Long.valueOf(docId));
    ...
    private void displayImage(String imagePath){
        if (imagePath != null){
            ...
            Toast.makeText(PayJoinActivity.this,"获取照片失败",Toast.LENGTH_SHORT).show();
        }
    }
